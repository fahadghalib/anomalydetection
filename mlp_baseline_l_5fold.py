"""M9: strong-MLP baseline WITHOUT the 4 engineered temporal features
(dt_prev, pos_delta, spd_delta, msg_rate_5s) -- the original 28 paper columns
only. Same architecture/protocol as M8 (src/mlp_baseline_5fold.py: 5 folds,
100 epochs, patience 20, ADASYN, weighted focal loss) and the same idea as
M6 (no_temporal_features) in the ablation study, but applied to the MLP
architecture instead of CNN-BiGRU-Attention.

Why: M8 (32 features, WITH temporal) beat M7 hugely on exactly the
temporal/repetition attack classes (Data Replay +17.1pt F1, DDoS +17.9pt) --
classes the temporal features were engineered for. This isolates whether
M8's advantage is purely architectural (flat MLP > pseudo-sequence CNN/BiGRU)
or partly riding on the temporal features it has just as much access to as
M7 does. Slicing to the first 28 columns matches data_pipeline.py's
convention: the 4 temporal features are always appended LAST.
"""
...
def run_mlp_no_temporal_5fold(n_folds: int = config.N_FOLDS, resume: bool = False):
    t_start = time.time()
    X32, y = load_train()
    X = X32[:, :N_FEATURES_NO_TEMPORAL]  # drop the 4 temporal columns (always appended last)
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(skf.split(X, y), start=1):
        ...
        report_path = config.RESULTS_DIR / f"{MODEL_PREFIX}{fold_idx}_val_report.json"
        if resume and model_path.exists() and scaler_path.exists() and report_path.exists():
            ...
            print(f"\n[fold {fold_idx}] already completed (resume=True) -- skipping, "
                  f"val_accuracy={fold_summaries[-1]['val_accuracy']}", flush=True)
            continue

        print(f"\n{'=' * 70}\nM9_mlp_no_temporal FOLD {fold_idx}/{n_folds}\n{'=' * 70}", flush=True)
        ...
        model = build_mlp_baseline(N_FEATURES_NO_TEMPORAL, config.N_CLASSES)
        model.compile(
        ...
        callbacks = [
            ...
            tf.keras.callbacks.CSVLogger(str(config.LOGS_DIR / f"{MODEL_PREFIX}{fold_idx}_history.csv")),
            EpochTimer(fold_idx),
        ]
        ...
        print(f"[fold {fold_idx}] done in {fold_summary['elapsed_sec'] / 60:.1f} min. "
              f"macro-recall={fold_summary['val_macro_recall']:.4f}", flush=True)

    with open(config.RESULTS_DIR / "mlp_no_temporal_5fold_summary.json", "w") as f:
    ...
    print(
        f"\nM9 5-fold complete in {(time.time() - t_start) / 3600:.2f} hours. "
...
if __name__ == "__main__":
    ...
    run_mlp_no_temporal_5fold(resume=resume)
